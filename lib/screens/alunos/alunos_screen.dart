...
class _AlunosScreenState extends State<AlunosScreen> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) setState(() { _alunos = lista; _loading = false; });
  }
  ...
  List<Aluno> get _filtrados {
    return _alunos.where((a) {
      ...
      final matchStatus = _filtroStatus == 'todos'
          ? true
          : _filtroStatus == 'ativo' ? a.ativo : !a.ativo;
      return matchBusca && matchFaixa && matchStatus;
    }).toList();
  }
  ...
  Future<void> _validar(Aluno a) async {
    await _repo.validar(a.id);
    _load();
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${a.nome} validado!'), backgroundColor: verdeEscuro));
  }
...
class _AlunoCard extends StatelessWidget {
  ...
  const _AlunoCard({required this.aluno, required this.onEdit, required this.onDelete, this.onValidar});
