...
class _PerfilScreenState extends State<PerfilScreen> {
  ...
  Future<void> _load() async {
    ...
    if (user == null) return;
    Aluno? aluno;
    if (user.email.isNotEmpty) aluno = await _alunoRepo.buscarPorEmail(user.email);
    List<Mensalidade> mens = [];
    if (aluno != null) mens = await _mensRepo.porAluno(aluno.id);
    if (mounted) setState(() { _aluno = aluno; _mensalidades = mens.take(6).toList(); _loading = false; });
  }
